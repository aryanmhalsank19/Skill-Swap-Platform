...
import { Star, MapPin, CheckCircle, ExternalLink, Clock, Users } from "lucide-react"
...
export function UserCard({ user }: UserCardProps) {
  ...
  return (
    <>
      <Card className="group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-card/50 backdrop-blur border-border/50 overflow-hidden">
        <CardContent className="p-6">
          ...
          <div className="flex items-start space-x-4 mb-6">
            <Avatar className="h-14 w-14 ring-2 ring-primary/20 group-hover:ring-primary/40 transition-all duration-300">
              ...
              <AvatarFallback className="bg-gradient-to-br from-primary to-green-400 text-white font-semibold">
                {user.name.charAt(0).toUpperCase()}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
              <Link href={`/users/${user.id}`} className="hover:underline group">
                ...
                {offeredSkills.slice(0, 3).map((skill) => (
                  <Badge
                    ...
                    className={`text-xs transition-all duration-200 ${
                      skill.is_verified || skill.verification_count > 0 
          ...
          {/* Social Links */}
          {(user.linkedin || user.github || user.personal_portfolio) && (
            <div className="flex items-center space-x-2 pt-2 border-t border-border/30">
              ...
              {user.personal_portfolio && (
                <Link 
                  href={user.personal_portfolio} 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="text-muted-foreground hover:text-primary transition-colors"
                >
